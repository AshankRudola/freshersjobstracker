#!/usr/bin/env python3
"""
Freshers Jobs Tracker Management CLI
...
def run_with_venv(cmd):
    ...
    if os.environ.get('VIRTUAL_ENV'):
        ...
        full_cmd = f"{VENV_PATH}\\Scripts\\activate.bat && {cmd}"
        return subprocess.run(full_cmd, shell=True, cwd=PROJECT_ROOT)

def cmd_start(args):
    """Start the Flask app"""
    print("Starting Freshers Jobs Tracker...")
    ...
    if args.bg:
        # Run in background
        print("Running in background... (http://127.0.0.1:5001)")
        result = run_with_venv(f"start /b python app.py > app.log 2>&1")
        ...
        result = run_with_venv("python app.py")

def cmd_stop(args):
    """Stop the Flask app"""
    if not APP_PID_FILE.exists():
        # Try to find and kill by name
        print("🛑 Stopping Freshers Jobs Tracker...")
        run_with_venv("taskkill /F /IM python.exe /T")
        print("✅ App stopped")
        return
...
def cmd_db_info(args):
    ...
    # Flagged jobs
    c.execute("SELECT COUNT(*) FROM jobs WHERE interested = 1")
...
def cmd_db_flagged(args):
    ...
    c = conn.cursor()
    c.execute("SELECT id, title, company, posted_date, comment FROM jobs WHERE interested = 1 ORDER BY company")
    jobs = c.fetchall()
    conn.close()
...
def cmd_install(args):
    ...
    result = run_with_venv(f"python -m pip install -r requirements.txt")
...
Examples:
  python manage.py start           # Start and watch logs
  python manage.py start --bg      # Start in background
  python manage.py stop            # Stop the app
  python manage.py db:info         # View DB stats
  python manage.py db:flagged      # View flagged jobs
  python manage.py db:clean        # Clear database
...
def main():
    parser = argparse.ArgumentParser(description='Freshers Jobs Tracker Management CLI', add_help=False)
